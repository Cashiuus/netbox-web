...
class BulkImportView(GetReturnURLMixin, BaseMultiObjectView):
    ...
    def get_or_init_instance(self, row):
        """
        Determine if the provided row data already exists in DB or if this is new data.
        """
    ...
    def create_and_update_objects(self, form, request):
        ...
        prefetch_ids = [record[import_id_field] for record in records if record.get(import_id_field)]
        # For this to work, I have to find a way to use my field var in a filter query statement
        # For now, hardcoding "name" for testing just to see if it works
        prefetched_objects = {
            record[import_id_field]: obj
            for obj in self.queryset.model.objects.filter(name__in=prefetch_ids)
        } if prefetch_ids else {}

        for i, record in enumerate(records, start=1):
            instance = None
            # object_id = int(record.pop('id')) if record.get('id') else None
            object_id = record.pop(import_id_field) if record.get(import_id_field) else None
            ...
            if object_id:
                try:
                    ...
                    params = {}
                    params[import_id_field] = record[import_id_field]
                    instance = self.get_queryset().get(**params)
                    # instance, new = self.get_or_init_instance(record)
                except KeyError:
